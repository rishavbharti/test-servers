            ...
            if (this.data === undefined) {
                ...
                setTimeout(() => {
                    ...
                    if (this.data.includes("/raw-echo HTTP/1.1")) {
                        return socket.end(
                            this.data.substring(this.data.indexOf("\r\n\r\n"))
                        );
                    }
                    ...
                    socket.write(
                        "raw-request: " + JSON.stringify(this.data) + "\r\n"
                    );
...
function createRedirectServer() {
    var server = http.createServer(function (req, res) {
        var redirectURL, urlTokens, responseCode, numberOfRedirects;
...
function createProxyServer(options) {
    ...
    server.on("request", function (req, res) {
        ...
        if (
            options.auth &&
            req.headers["proxy-authorization"] !== proxyAuthHeader
        ) {
            res.writeHead(407);
    ...
    function canonicalizeHeaders(headers) {
        ...
        options.headersToSign.forEach(function (headerName) {
            ...
            formattedHeaders.push(
                `${headerName}:${headerValue.trim().replace(/\s+/g, " ")}`
            );
        });
    ...
    function authHandler(req, res, body) {
        ...
        if (
            !(
                ...
                /[0-9]{8}T[0-9]{2}:[0-9]{2}:[0-9]{2}\+0000/.test(
        ...
        authParams.body =
            body && body.slice && body.subarray(0, options.maxBodySize);
...
function createNTLMServer(options) {
    ...
    var type2Message =
        ...
        handler = function (req, res) {
            ...
            if (
                authHeaders &&
                authHeaders.startsWith(type1Message.slice(0, 20))
            ) {
                res.writeHead(401, {
            ...
            // because I don't know how.
                ...
                authHeaders &&
                authHeaders.startsWith(type3Message.slice(0, 100))
            ) {
            ...
            // no valid auth headers, ask for type1 message
                ...
                !authHeaders &&
                authenticatedRequestId.includes(executionId)
            ) {
                res.writeHead(200);
            } else {
                res.writeHead(401, {
                ...
                options.debug &&
                    console.info("401: got no authorization header");
            }
...
function createDigestServer(options) {
    ...
    passport.use(
        new DigestStrategy({ qop: "auth" }, function (username, done) {
...
function createWSSServer(opts) {
    ...
    server.on("listening", function () {
        server.port = this.address().port;
        server.url = "wss://localhost:" + server.port;
    });

    enableServerDestroy(server);
    server.wss = wss; // Attach wss for external access if needed
    return server;
}
